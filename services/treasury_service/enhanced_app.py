...
from services.treasury_service.graph.graph import build_graph
from services.treasury_service.graph.types import AgentState
...
# --- Refactored chat endpoint to use LangGraph workflow ---
@app.post("/chat/message")
async def chat_message(message: dict):
    """Treasury chat endpoint using LangGraph workflow."""
    # Prepare initial state for the workflow
    # Support both "message" and "query" keys
    user_query = message.get("query") or message.get("message") or ""
    state = AgentState(query=user_query)
    # Run the workflow
    result = graph.invoke(state)
    # Add to chat history
    now = datetime.now().isoformat()
    user_entry = {"id": f"msg-{int(datetime.now().timestamp()*1000)}-u", "role": "user", "content": user_query, "timestamp": now}
    _add_history(user_entry)
    assistant_entry = {"id": f"msg-{int(datetime.now().timestamp()*1000)}-a", "role": "assistant", "content": result.get("response", str(result)), "timestamp": datetime.now().isoformat()}
